...
config: {
    name: "bank",
    version: "1.9",
    author: "AYAN",/*don't change my credit please 😠*/
    countDown: 5,
    role: 0,
    shortDescription: {
      vi: "",
  ...
  onStart: async function ({ args, message, event, usersData }) {
    ...
    const bankData = JSON.parse(fs.readFileSync("ariyan-bank.json", "utf8"));

    if (!bankData[user]) {
       bankData[user] = { bank: 0, lastInterestClaimed: Date.now() };
      fs.writeFile("ariyan-bank.json", JSON.stringify(bankData), (err) => {
        if (err) throw err;
      });
    }
